      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-8 bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-slate-700">Price Basket</h3>
            <span className="ml-auto text-xs text-slate-400">Select products to see their combined total</span>
            {basketIds.length > 0 && (
              <button
          ...
          <div className="relative mb-4">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 rounded-md border border-slate-200 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400"
              />
              {basketSearchQuery && (
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-10 overflow-hidden">
                {basketSearchResults.map(p => (
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-500 flex-shrink-0">₹{p.price.toFixed(2)}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          {basketItems.length === 0 ? (
            <p className="text-sm text-slate-400 py-4 text-center">Search and add products above to calculate combined price.</p>
          ) : (
            <div className="space-y-2">
              {basketItems.map(p => (
                <div key={p.id} className="flex items-center gap-3 px-3 py-2.5 rounded-md border border-slate-100 hover:border-slate-200 transition-colors">
                  <div className="w-7 h-7 flex-shrink-0 rounded-md bg-slate-100 overflow-hidden flex items-center justify-center text-xs font-semibold text-slate-600">
                    {p.image_url
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {p.company && <div className="text-xs text-slate-400 truncate">{p.company}</div>}
                  </div>
                  <div className="text-sm font-semibold text-slate-800 flex-shrink-0">₹{p.price.toFixed(2)}</div>
                  <button
              ...
              <div className="flex items-center justify-between pt-3 mt-1 border-t border-slate-100">
                <div className="text-xs text-slate-500">{basketItems.length} product{basketItems.length !== 1 ? 's' : ''} in basket</div>
